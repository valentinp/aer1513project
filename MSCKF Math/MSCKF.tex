...
\begin{align}
\bbm h_1 (\alpha, \beta, \rho) \\ h_2 (\alpha, \beta, \rho) \\ h_3 (\alpha, \beta, \rho) \ebm = \mbf C_{in} \bbm \alpha \\ \beta \\ 1 \ebm + \rho  \mbf p^{C_i}_{C_1}
\end{align}
The camera measurement error can then be written as:
\begin{align}
\mbf e = \hat{\mbf z} - \frac{1}{h_3 } \bbm h_1 \\ h_2 \ebm
\end{align}
...
where
\begin{align}
\mbf E := \frac{\partial \mbf e}{\partial \mbf x} \in \Real^{2 N \times 3 }, \quad \mbf W := \bbm \mbf R^{-1} & & \\ & \ddots & \\ & & \mbf R^{-1} \ebm \in \Real^{2 N \times 2 N}
\end{align}
