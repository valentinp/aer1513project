...
\thanks{All authors are at the Institute for Aerospace Studies,
...
In this work, we compare and contrast two modern solutions: the Sliding Window Filter (SWF) and the Multi-State Constraint Kalman Filter (MSCKF) \cite{Mourikis:2006:TechReport,Mourikis:2007:ICRA}.
...
Visual-inertial navigation systems (VINS) have been applied broadly in robotics \cite{Mourikis:2007:ICRA, Sibley:2010:JFR, Huster:2002:OC, Kottas:2013:IROS, Kim:2007:RSS, Li:2013coa,  You:2001:VR}. 
As such, there is a considerable body of work on the subject and a wide range of optimal filtering algorithms for the Camera-IMU sensor pair. 
These techniques are often characterized as either \textit{loosely coupled}, when image and IMU measurements are processed individually to produce pose estimates before being fused into a single estimate, or \textit{tightly coupled}, when all information is processed together in a single estimation paradigm.  
The decoupling of inertial and visual measurements in the former case necessarily leads to inexpensive computations, but at the cost of information: processing the IMU measurements separate from the filtering framework makes optimal estimation of biases impossible\cite{Li:2013coa}. 
The filters presented in this work fall in the latter category; tightly coupled methods are preferred when the goal is accurate, consistent visual-inertial navigation. 

In both cases, the optimal filtering algorithm used is typically based on the Extended Kalman filter (EKF)(\cite{Huster:2002:OC, You:2001:VR, Kottas:2013:RSS, Kim:2007:RSS, Mourikis:2007:ICRA}), though methods exist that employ unscented Kalman filters \cite{Ebcin:2007:ION}, particle filters \cite{Fox:1999:JAIR, Pupilli:2005:BMVC}, or batch optimization methods\cite{Triggs:2000va}, \cite{Strelow:2004:IJR}. 
The EKF-SLAM is an efficient recursive algorithm for small-scale, online tasks, but maintaining the entire map as part of the state is a weakness for navigation over long distances. Indeed, the computational complexity of such algorithm scales far too poorly with the number of features to be applied naively to this problem.
Additionally, EKF-based methods for visual-inertial navigation are fundamentally inconsistent, that is, the state uncertainties are underestimated. It was shown in \cite{Li:2013coa} that the Jacobians in the linearized model of a VINS have different observability properties than the actual nonlinear system, severely reducing the reliability of such estimators.
...
It also takes advantage of \textit{delayed linearization} in attempt to minimize linearization inaccuracies. The SWF analyzed in this work is similar as a both a recursive and batch approach, but unlike the VSDF it incorporates a dynamic model and applies batch optimization. 
The SWF is a constant time algorithm as it actively marginalizes states and landmarks to maintain a state vector remains of constant size. Batch optimization of a window of states is suboptimal but still leads to exceptional accuracy for its cost, making it applicable on delicate, large scale operations such as planetary landing\cite{Sibley:2010:JFR}. 
The MSCKF is also a sliding window approach, taking advantage of delayed linearization and exploiting the computational efficiency of an EKF. It has recently been used to achieve accurate, online position tracking onboard a smartphone \cite{Li:2013:ICRA}.

In this work, we present an experimental comparison of the SWF and MSCKF using different datasets consisting of IMU and camera data with accurate ground truth for both sensor motion and landmark positions. In order to provide context for the MSCKF and SWF, we first outline the EKF approach to the SLAM problem.
...
The optimization problem in the SWF is typically solved as a non-linear least squares problem using Gauss-Newton (GN) optimization over a sliding window of $K$ poses $\Vec x_{k\in[1,K]}$ observing $M$ features $f_{j\in[1,M]}$: 
\begin{equation}
...
\end{align}
The linear least-squares system then becomes
\begin{align}
...
\begin{figure}
    \centering
    \subfigure {
        \includegraphics[width=0.45\textwidth]{figs/CRV/NST_RMSE_trans}
        \label{fig:nullspace:trans_err}
    }
    ~
    \subfigure {
        \includegraphics[width=0.45\textwidth]{figs/CRV/NST_RMSE_rot}
...
\begin{figure*}
    \centering
    \subfigure {
        \includegraphics[width=0.45\textwidth]{figs/6-Way-Comparison-500-1000-Trans}
        \label{fig:comp_1:trans_err}
    }
    ~
    \subfigure {
        \includegraphics[width=0.45\textwidth]{figs/6-Way-Comparison-500-1000-Rot}
...
\begin{figure*}
    \centering
    \subfigure {
        \includegraphics[width=0.45\textwidth]{figs/6-Way-Comparison-1215-1715-Trans}
        \label{fig:comp_2:trans_err}
    }
    ~
    \subfigure {
        \includegraphics[width=0.45\textwidth]{figs/6-Way-Comparison-1215-1715-Rot}
...
In each dataset, the IMU data remained unchanged from the original dataset and we corrupted the synthetic camera measurements with zero-mean Gaussian noise.
...
\begin{figure*}
    \centering
    \subfigure {
        \includegraphics[width=0.45\textwidth]{figs/RMSE-Comparison-Feat-Density-1215-1715-Trans}
        \label{fig:comp_3:trans_err}
    }
    ~
    \subfigure {
        \includegraphics[width=0.45\textwidth]{figs/RMSE-Comparison-Feat-Density-1215-1715-Rot}
...
Table \ref{tab:rmse_nees} provides summary statistics of each algorithm's performance on the interval shown in Figure \ref{fig:comp_3}.
As expected, the SWF outperforms the MSCKF in terms of average RMSE on both rotation estimation and position estimation.
...
\begin{table}
    \centering
    \caption{Comparison of Average Root Mean Squared Error (ARMSE), Average Normalized Estimation Error Squared (ANEES), and Compute Time of IMU integration, MSCKF, and SWF on synthetic datasets for the interval $k\in[1215,1715]$.}
