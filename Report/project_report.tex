...
The correlation between the measurement error and the state causes the filter estimates to drift further from the true values.
...
\begin{figure}
    ...
    \caption{Effect of nullspace projection on MSCKF performance in a synthetic 100-feature dataset. The correlation between the measurement error and the state causes the filter estimates to drift further from the true values.}
    \label{fig:nullspace}
\end{figure}
...
\begin{figure}
    ...
    \caption{Comparison of cumulative Root Mean Squared Error (RMSE) of MSCKF, SWF, and IMU integration for three different synthetic maps generated based on the IMU data in the dataset interval $k\in[1215,1715]$. The numbers next to ``MSCKF'' and ``SWF'' refer to the number of features in the dataset (40, 60 or 100).}
    \label{fig:comp_3}
\end{figure}
