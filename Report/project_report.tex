...
A complicating factor in the general form of this problem is the absence of a known map of landmarks from which the camera can generate measurements.
...
What follows is a discussion of two common solutions, the Extended Kalman Filter (EKF) and the Sliding Window Filter (SWF), and a third hybrid solution, the Multi-State Constraint Kalman Filter (MSCKF), that combines the strengths of both.
...
This behavior is due to the fact that the dimension of the state grows linearly with the number of landmarks, and the computational cost of inverting the state covariance matrix while computing the Kalman gain is cubic in the dimension of the state.
...
At time $k$, the full state of the MSCKF consists of the current IMU state estimate, and estimates of $N$ past camera poses in which active feature tracks were visible:
\begin{align}
...
\begin{align}
    \tilde{\Vec{x}}_k &= \bbm \tilde{\Vec{x}}_{I,k}^T & \boldsymbol{\delta\theta}_{C_1}^T & \tilde{\Vec{p}}_G^{C_1 G \hspace{0.3em} T} & \hdots & \boldsymbol{\delta\theta}_{C_N}^T & \tilde{\Vec{p}}_G^{C_N G \hspace{0.3em} T} \ebm ^T
\end{align}
where
\begin{align}
    \tilde{\Vec{x}}_{I,k} &= \bbm \boldsymbol{\delta\theta}_I^T & \tilde{\Vec{b}}_{\boldsymbol{\omega},k}^T & \tilde{\Vec{b}}_{\Vec{v},k}^T & \tilde{\Vec{p}}_{G,k}^{IG \hspace{0.3em} T} \ebm^T
\end{align}
is the 12-dimensional IMU error state.
In the above, $\tilde{x}$ denotes the difference between the true value and the estimated value of the quantity $x$.
...
\end{align}
where $\hat{\Vec{P}}_{II,k}$ is the $12\times12$ covariance matrix of the current IMU state, $\hat{\Vec{P}}_{CC,k}$ is the $6N\times6N$ covariance matrix of the camera poses, and $\hat{\Vec{P}}_{IC,k}$ is the $12\times6N$ cross-correlation between the current IMU state and the camera poses.

\subsection{MSCKF State Augmentation}
When a new camera image becomes available, the MSCKF state must be augmented with the current camera state.
We obtain the camera state by applying the known transformation $\left(\Vec{q}_{CI}, \Vec{p}_I^{CI}\right)$ to a copy of the current IMU state:
\begin{align}
    \hat{\Vec{q}}_{C_{N+1} G} &= \Vec{q}_{CI} \otimes \hat{\Vec{q}}_{IG,k} \\
    \hat{\Vec{p}}_G^{C_{N+1}G} &= \hat{\Vec{p}}_G^{IG} + \hat{\Vec{C}}_{IG,k} \hat{\Vec{p}}_I^{CI}
\end{align}
...
\begin{align}
    \hat{\Vec{x}}_k &\leftarrow \bbm \hat{\Vec{x}}_k^T & \hat{\Vec{q}}_{C_{N+1} G}^T & \left(\hat{\Vec{p}}_G^{C_{N+1}G}\right)^T \ebm^T
\end{align}
...
The dataset consists of a rigidly attached stereo camera and IMU (Figure \ref{fig:sensorhead}) observing a set of 20 landmarks while moving along an arbitrary 3D path.
