...
We present experimental comparisons of the MSCKF and SWF on data from a moving hand-held sensor rig, as well as several traverses from the KITTI dataset.
In particular, we compare the accuracy of the two filters and analyze the effect of feature track length and feature density on the performance of each.
In general, our results show the SWF to be at least as accurate the MSCKF, however the MSCKF is significantly more efficient and improves in accuracy as more features are tracked. 
\end{abstract}
...
It also takes advantage of \textit{delayed linearization} in attempt to minimize linearization errors. The SWF analyzed in this work is similar as a both a recursive and batch approach, but unlike the VSDF it incorporates a motion model and applies batch optimization. 
...
We compared the performance of the MSCKF and SWF algorithms on two intervals in the ``Starry Night'' dataset from the University of Toronto Institute for Aerospace Studies (UTIAS), as well as several runs from the KITTI Dataset \cite{Geiger:2013:IJRR}.
...
This dataset is well-suited to evaluating SLAM algorithms since accurate ground truth from a Vicon motion capture system is available for both the sensor head motion and the feature positions.
...
\begin{figure*}
    \centering
    \subfigure[At least three features are visible in $64\%$ of the interval $k\in(500,1000)$. The number of visible features is frequently exceeds ten.] {
        \includegraphics[width=0.45\textwidth]{figs/RMSE-Comparison-WindowSize-500-1000}
        \label{fig:comp_1}
    }
    ~
    \subfigure[At least three features are visible in only $56\%$ of the interval $k\in(1215,1715)$. The number of visible features rarely exceeds ten.] {
...
\begin{figure*}
    \centering
    \subfigure[RMSE on the \texttt{2011\_09\_26\_drive\_0001} traverse.] {
        \centering
        \includegraphics[width=0.45\textwidth]{figs/kitti/RMSE_2011_09_26_drive_0001}
    }
    ~
    \subfigure[RMSE on the \texttt{2011\_09\_26\_drive\_0036} traverse.] {
        \centering
        \includegraphics[width=0.45\textwidth]{figs/kitti/RMSE_2011_09_26_drive_0036}
    }

    \subfigure[RMSE on the \texttt{2011\_09\_26\_drive\_0051} traverse.] {
        \centering
        \includegraphics[width=0.45\textwidth]{figs/kitti/RMSE_2011_09_26_drive_0051}
    }
    ~
    \subfigure[RMSE on the \texttt{2011\_09\_26\_drive\_0095} traverse.] {
        \centering
        \includegraphics[width=0.45\textwidth]{figs/kitti/RMSE_2011_09_26_drive_0095}
    }
    \caption{RMSE over four traverses from the KITTI dataset \cite{Geiger:2013:IJRR} totaling 534 m of urban driving.}
    \label{fig:kitti}
\end{figure*}
