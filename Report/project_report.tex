...
Since the error in the feature positions \textit{is} correlated to the error in the state, $\Vec{r}^{(j)}$ cannot be used in the EKF directly.
Figure \ref{fig:nullspace} shows the effect of attempting to use this error directly in the EKF.
The correlation between the measurement error and the state causes the filter to behave very poorly after a small number of time steps.
...
\subsection{Interval 1: Many visible features}
...
\begin{figure}
    ...
    \caption{Comparison of MSCKF, SWF, and IMU integration for multiple parameter settings on the dataset interval $k\in[500,1000]$. The numbers next to ``MSCKF'' in the legend refer to the minimum and maximum feature tracks lengths before an EKF update is triggered, while the number next to ``SWF'' refers to the number of states in the sliding window.}
    \label{fig:comp_1}
\end{figure}

\subsection{Interval 2: Fewer visible features}
...
\begin{figure}
    ...
    \caption{Comparison of MSCKF, SWF, and IMU integration for multiple parameter settings on the dataset interval $k\in[1215,1715]$. The numbers next to ``MSCKF'' in the legend refer to the minimum and maximum feature tracks lengths before an EKF update is triggered, while the number next to ``SWF'' refers to the number of states in the sliding window.}
    \label{fig:comp_2}
\end{figure}
