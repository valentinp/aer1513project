...
\author{Lee E Clement, Valentin Peretroukhin, Jacob Lambert, and Jonathan Kelly% <-this % stops a space
%\thanks{*This work was not supported by any organization}% <-this % stops a space
\thanks{Lee E Clement and Valentin Peretroukhin jointly assert first authorship. All authors are at the Institute for Aerospace Studies, University of Toronto, Canada
...
For this task, the combination of visual and inertial sensors is an inexpensive, compact, and complementary hardware suite that can be used on many types of vehicles.
...
However, the MSCKF is computationally cheaper, has good consistency properties, and improves in accuracy as more features are tracked. 
\end{abstract}
...
Since an IMU measures linear accelerations and rotational velocities, these values must be integrated to arrive at a new pose estimate.
...
In these cases, IMU data can be relied upon more heavily when estimating egomotion.
...
Any solution must therefore solve a Simultaneous Localization and Mapping (SLAM) problem, although the importance placed on mapping may vary from algorithm to algorithm.
...
We also conduct a more extensive characterization of the sensitivity of the MSCKF to certain tuning parameters and compare both algorithms using data from a hand-held sensor rig that mimics the more arbitrary motion of a micro aerial vehicle (MAV).

\begin{figure}[t]
    \vspace{0.25cm}
    \centering
    \includegraphics[width=0.45\textwidth]{figs/sensorhead}
    \caption{The hand-held sensor head used in our experiments with the ``Starry Night'' dataset. The IMU reports translational and rotational velocities, while the stereo camera observes point features. Since we are comparing monocular algorithms, we used measurements from the left camera of the stereo pair only.}
...
For both tightly coupled and loosely coupled VINS, a popular estimator is the Extended Kalman filter (EKF) or one of its variants \cite{Huster:2002:OC, You:2001:VR, Kottas:2013:RSS, Kim:2007:RSS, Mourikis:2007:ICRA}, though methods also exist that employ unscented Kalman filters \cite{Ebcin:2007:ION}, particle filters \cite{Fox:1999:JAIR, Pupilli:2005:BMVC}, or batch optimization methods\cite{Triggs:2000va,Strelow:2004:IJR}. 

EKF-SLAM is an efficient recursive algorithm for small-scale, online tasks, but maintaining the entire map as part of the state is computationally expensive when navigating over long distances.
...
Prior work has shown that this is due to the fact that the Jacobians in the linearized model of a VINS have different observability properties than the actual nonlinear system \cite{Huang:2010:IJRR,Li:2013coa}.
...
The shortcomings of the EKF motivate the use of filters that operate on a larger subset of the problem. Algorithms operating in constant time with respect to map size are particularly desirable.
% The Variable State Dimension Filter (VSDF)\cite{McLauchlan:1993:SPIE} is an early example of such algorithms. It applied a Kalman filter over a sliding window of states to achieve linear complexity with the number of features, taking advantage of \textit{delayed linearization} in attempt to minimize linearization errors.
...
Except where otherwise noted (i.e., state parametrization in Section \ref{sec:state_parametrization} and integration method in Section \ref{sec:imu_prop}), we implemented the MSCKF as presented in \cite{Mourikis:2006:TechReport, Mourikis:2007:ICRA}.
...
\end{align}
where $\hat{\Vec{P}}_{II,k}$ is the $12\times12$ covariance matrix of the current IMU state, $\hat{\Vec{P}}_{CC,k}$ is the $6N\times6N$ covariance matrix of the camera poses, and $\hat{\Vec{P}}_{IC,k}$ is the $12\times6N$ cross-covariance between the current IMU state and the past camera poses.
...
The MSCKF may therefore be better suited to platforms with limited computational resources, particularly if they are operating in feature-rich environments.
...
We extracted between 50 and 100 salient point features from the left camera in the stereo pair using Speeded Up Robust Features (SURF) \cite{Bay:2008:CVIU} and tracked them temporally using Kanade-Lucas-Tomasi (KLT) tracking \cite{Tomasi:91}.
...
% Like the ``Starry Night'' datasets, we initialized 3D points in both filters using the inverse-depth batch optimization of Section \ref{sec:featPosinvDepth}.
...
\begin{figure*}
    \centering
    \subfigure[Translational RMSE on KITTI traverse \texttt{0001}.] {
        \centering
        \includegraphics[width=0.47\textwidth]{figs/kitti/RMSE_trans_2011_09_26_drive_0001}
    }
    ~
    \subfigure[Translational RMSE on KITTI traverse \texttt{0036}.] {
        \centering
        \includegraphics[width=0.47\textwidth]{figs/kitti/RMSE_trans_2011_09_26_drive_0036}
    }

    \subfigure[Translational RMSE on KITTI traverse \texttt{0051}.] {
        \centering
        \includegraphics[width=0.47\textwidth]{figs/kitti/RMSE_trans_2011_09_26_drive_0051}
    }
    ~
    \subfigure[Translational RMSE on KITTI traverse \texttt{0095}.] {
        \centering
        \includegraphics[width=0.47\textwidth]{figs/kitti/RMSE_trans_2011_09_26_drive_0095}
    }
    \caption{Translational RMSE over four traverses from the KITTI dataset \cite{Geiger:2013:IJRR} totaling 534 m of urban driving.}
    \label{fig:kitti}
