...
\begin{equation}
\delta\Vec{x} := \bbm \delta \Vec x_0 \\ \vdots \\ \delta \Vec x_K \\ \delta \Vec f_1 \\ \vdots \\ \delta \Vec f_N\ebm,
\end{equation}
...
\begin{equation}
\Vec G_{\Vec{x}, k}^j := \pd{\Vec g}{ \Vec p}\Bigr|_{\Vec p = \bar{\Vec p}_{c_k}^{p_j c_k}} \bbm -\Vec C_{cv} \Vec C_{v_k i} & \Vec C_{cv} (\Vec C_{v_k i} (\boldsymbol{\rho}_i^{p_j i} - \Vec r_i^{v_k i} ) )^\times  \ebm\end{equation}

\begin{equation}
\quad \Vec G_{\Vec x, k} := \bbm \Vec G_{\Vec x, k}^1 \\ \vdots \\ \Vec G_{\Vec x, k}^M \ebm
\end{equation}

\begin{equation}
\Vec G_{\Vec f, k}^j := \pd{\Vec g}{ \Vec p}\Bigr|_{\Vec p = \bar{\Vec p}_{c_k}^{p_j c_k}} \bbm \Vec C_{cv} \Vec C_{v_k i}\ebm\end{equation}

\begin{equation}
\quad \Vec G_{\Vec f, k} := \underbrace{\bbm \Vec G_{\Vec f, k}^1 & & \\ & & \Vec G_{\Vec f, k}^2 & \\ & & \vdots &  \\ & & & \Vec G_{\Vec f, k}^M \ebm}_{\text{$j$th block column position given by feature id $n$}} \in \Real^{4M \times 3N}
\end{equation}

and,
\begin{equation}
\bar{\Vec p}_{c_k}^{p_j c_k} := \bbm x \\ y \\ z \ebm = \Vec C_{cv} \left( \Vec C_{v_k i} (\boldsymbol{\rho}_i^{p_j i} - \Vec r_i^{v_k i} )   - \boldsymbol \rho_v^{cv} \right),
\end{equation}

\begin{equation}
\pd{\Vec g}{ \Vec p}\Bigr|_{\Vec p = \bar{\Vec p}_{c_k}^{p_j c_k}} = \bbm \frac{f_u}{z} & 0 & -\frac{1}{z^2} f_u x \\  0 & \frac{f_v}{z} & -\frac{1}{z^2} f_v y  \\ \frac{f_u}{z} & 0 & -\frac{1}{z^2} f_u (x-b) \\  0 & \frac{f_v}{z} & -\frac{1}{z^2} f_v y\ebm.
\end{equation}
or for monocular,
\begin{equation}
\pd{\Vec g}{ \Vec p}\Bigr|_{\Vec p = \bar{\Vec p}_{c_k}^{p_j c_k}} = \bbm \frac{f_u}{z} & 0 & -\frac{1}{z^2} f_u x \\  0 & \frac{f_v}{z} & -\frac{1}{z^2} f_v y \ebm.
\end{equation}
...
\end{align}
with $\Vec{R}_i^{(j)} = \text{diag}\left\{\sigma^2_u, \sigma^2_v\right\}$.
