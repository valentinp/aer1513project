...
In loosely coupled systems, image and IMU measurements are processed separately before being fused into a single estimate, while tightly coupled systems process all information together.  
...
\end{align}
where $\Delta T$ is the IMU sampling period, and the state transition matrix $\boldsymbol{\Phi}\left(t_k + \Delta T, t_k \right)$ is obtained from
\begin{align} \label{eq:statetransition}
	\boldsymbol{\Phi}\left(t_k + \Delta T, t_k \right) &= \Vec{1}_{12} + \Vec{F} \Delta T.
\end{align}

We obtain the predicted IMU state covariance $\hat{\Vec{P}}^-_{II,k+1}$ according to
\begin{align}
