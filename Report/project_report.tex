...
The Jacobians $\Vec{F}$, $\Vec{G}$ are given by
\begin{align}
...
\Vec{G} &=  \bbm    -\Vec{1}_3 & \Vec{0}_{3\times3} & \Vec{0}_{3\times3} & \Vec{0}_{3\times3} \\
            ...
            \ebm ,
\end{align}
and $\Vec{Q}_I$ is the covariance of the IMU measurement noise.
...
Since $\Vec{H}_{f}^{(j)}$ has full column rank, $\Vec{A}$ has dimension $\left(2M_j - 3\right) \times 3$ and $\Vec{r}_o^{(j)}$ has dimension $\left(2M_j - 3\right) \times 1$.
Furthermore, because $\Vec{A}$ is semi-unitary, the covariance matrix of $\Vec{n}_o^{(j)}$ is given by $\Vec{R}_o = \sigma^2_{\text{im}}\Vec{A}^T\Vec{A} = \sigma^2_{\text{im}}\Vec{1}_{2M_j - 3}$.
...
Substituting this result into Equation \ref{eq:residual_nullspace_stacked} and pre-multiplying by $\bbm \Vec{Q}_1 & \Vec{Q}_2 \ebm ^T$, we obtain
\begin{align}
...
\end{align} 
The covariance matrix of $\Vec{n}_n$ is given by $\Vec{R}_n = \Vec{Q}_1^T \Vec{R}_o \Vec{Q}_1 = \sigma^2_\text{im} \Vec{1}_r$ where $r$ is the number of columns in $\Vec{Q}_1$.
